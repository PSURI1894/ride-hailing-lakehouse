...
def delivery_report(err, msg):
    if err is not None:
        ...
        logger.debug(
            f"Trip record {msg.key()} successfully produced to {msg.topic()} [{msg.partition()}] at offset {msg.offset()}"
        )
...
def main():
    ...
    for index, row in df.iterrows():
        ...
        record = {
            ...
            "passenger_count": (
                int(row.get("passenger_count"))
                if pd.notna(row.get("passenger_count"))
                else None
            ),
            "trip_distance": float(row.get("trip_distance") or 0.0),
            "RatecodeID": (
                int(row.get("RatecodeID")) if pd.notna(row.get("RatecodeID")) else None
            ),
            "store_and_fwd_flag": (
                str(row.get("store_and_fwd_flag"))
                if pd.notna(row.get("store_and_fwd_flag"))
                else None
            ),
            ...
            "congestion_surcharge": (
                float(row.get("congestion_surcharge"))
                if pd.notna(row.get("congestion_surcharge"))
                else None
            ),
            "airport_fee": (
                float(row.get("Airport_fee"))
                if pd.notna(row.get("Airport_fee"))
                else None
            ),
        }
